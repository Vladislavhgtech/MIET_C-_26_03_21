...
namespace MietMatrixTri
{
    class Matrix
    {
        ...
        private List<double> mainDiag; // главная диагональ
        private List<double> aboveMainDiag; // над главной диагональю
        private List<double> belowMainDiag; // под главной диагональю
        
        // Трехдиагональная матрица - квадратная, поэтому передаём в конструктор размерность
        public Matrix(int _dim)
        {
            // если размерность меньше 2, выбрасываем исключение
            ...
            // матрица определяется тремя массивами
        ...
        // ещё один констуктор, если нам известна главная диагональ
        public Matrix(int _dim, params double[] _diag)
        {
            if (_dim < 2)
                throw new Exception("Dimension must be greater than 1");
            Dimension = _dim;
        ...
        // конструктор с определенной главной диагональю и диагоналями выше и ниже главной
        public Matrix(int _dim, double[] _aboveMainDiag, double[] _mainDiag, double[] _belowMainDiag)
        {
            if (_dim < 3)
        ...
        {
            // описание решения системы в приложениии 1 к лабораторной
